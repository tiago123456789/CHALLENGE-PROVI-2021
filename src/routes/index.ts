...
export default (app: Express) => {
    ...
    app.post("/albums", upload.fields([ { name: "image", maxCount: 1 }, { name: 'musics', maxCount: 20 } ]), albumEndpoint.create)
    app.get("/albums", albumEndpoint.findAll) 
    app.get("/albums/:id", albumEndpoint.findById)
    app.put("/albums/:id", upload.fields([ { name: "image", maxCount: 1 }, { name: 'musics', maxCount: 20 } ]), albumEndpoint.update)
